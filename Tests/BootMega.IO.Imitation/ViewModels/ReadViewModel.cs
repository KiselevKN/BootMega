...
namespace BootMega.IO.Imitation.ViewModels
{
    public class ReadViewModel : BaseViewModel
    {
        ...
        public ReadViewModel(string portName, SerialPortSettings serialPortSettings, ILoggerFacade logger,
            MemoryType type, IMemory memory)
            : base(portName, serialPortSettings, logger)
        {
            Text = "Read " + ((type == MemoryType.FLASH) ? "FLASH" : "EEPROM");

            taskm = new TaskManager<bool>((taskManager) =>
            {
                return (new ReadPageWorkerSlave(portName, serialPortSettings, logger, type, memory).Run(taskManager));
            });
